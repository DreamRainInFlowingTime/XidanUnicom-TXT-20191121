...
def getJobElement(num,State='failed'):
    ...
    try:
        ...
        #traceback.print_exc()
        print("server"+str(num)+"请求被拒绝了\n")
        return jobs
    except Exception as e:
        print("server"+str(num)+"没抓到\n")
        #print(repr(e)+"-------")
        #print(str(e)+"------")
    ...
    for jobElement in jobElements:
        ...
        try:
            if (sqlElementText == "空任务名"):
                ...
                #sqlTextMatch = re.search(".*glkjoin.*",sqlElementText)
                #sqlText = sqlTextMatch.group()
        except AttributeError:
...
#获取任务运行至 某个时间点
                ...
                if (sqlElementText == "空任务名"):
                    ...
                    try:
                        sqlText = re.search("into\s(.*?)_str",sqlElementText,re.IGNORECASE).group(1)
                    except AttributeError:
                        continue
                if('context_info' in sqlText):
...
def run():
    ...
    #list = getID_fail(2)
    global CLUSTER_URL
    HUHEAliveServerIndexList=[1,2,3,6,7,8,11,12,13,16,17,18,19,20,21]
    #HUHEAliveServerIndexList=[1,2,3,4,6,7,8,9,11,12,13,14,16]
    #HUHEAliveServerIndexList=[1,2,6,16,20]
