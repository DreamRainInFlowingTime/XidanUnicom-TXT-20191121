...
CLUSTER_URL={ClusterLocation.HUHE:{0:ClusterLocation.HUHE,1:'http://10.162.235.40:4044/jobs?page=1&items=1000&status={0}',2:'http://10.162.235.41:4044/jobs?page=1&items=1000&status={0}'}}
    ...
    for num in nums:
        ...
        for job in jobs:
            ...
            if (CLUSTER_URL[0] == ClusterLocation.LANGFANG):
                ...
                isFailed = job.xpath("./td[8]//span/text()")
                isStopped = job.xpath("./td[7]//span/text()")
            else:
                #这里应该抛出个异常以后再说吧。。
                pass
            #print(isFailed)
...
#得到jobElements的sql
def  getSQL(serverNums:list,State='failed'):
    ...
    for jobElement in jobElements:
        ...
        if(CLUSTER_URL[0] == ClusterLocation.HUHE):
            sqlElementText = jobElement.xpath("./td[3]/div/div/em/text()")[0]
